...
def indices_of_k(arr, k):
    ...
    '''
    # print(arr)
    # print("X",np.argwhere(arr == k).flatten())
...
def fast_manhattan(x, y):
    ...
    # print("A", x)
    # print("B", y)
    # print("R", np.sum(np.absolute(x[:, np.newaxis] - y), axis = 1))
    # broadcast
    # return np.absolute(x[:, np.newaxis])
    return np.transpose(np.sum(np.absolute(x - y[:, np.newaxis]), axis = 2))
    # raise NotImplementedError
